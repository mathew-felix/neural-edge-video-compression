...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the isolated ROI-budgeting flow in a Colab-like GPU environment")
    parser.add_argument("--repo-root", type=str, default="/content/neural-edge-video-compression", help="Repo root")
    parser.add_argument("--video", type=str, required=True, help="Absolute or relative path to the input test clip")
    parser.add_argument(
        "--roi-target-kbps",
        type=float,
        default=150.0,
    ...
    parser.add_argument(
        ...
        type=str,
        default="research/roi_budgeting/configs/colab.yaml",
        help="Template research config to materialize into a runtime config",
    )
    parser.add_argument(
        "--compression-config-template",
        type=str,
        default="configs/gpu/compression.yaml",
        help="Template compression config used for baseline frame-removal artifacts",
    )
    parser.add_argument(
        "--runtime-research-config",
        type=str,
        default="research/roi_budgeting/configs/colab_runtime.yaml",
        help="Output path for the generated runtime research config",
    )
    parser.add_argument(
        "--runtime-compression-config",
        type=str,
        default="research/roi_budgeting/configs/colab_runtime_compression.yaml",
    ...
    parser.add_argument(
        "--allow-proxy",
        action="store_true",
        help="Allow a labeled AMT proxy manifest when CUDA AMT probing is unavailable",
    )
...
def main() -> None:
    ...
    compression_cfg.setdefault("roi_detection", {}).setdefault("runtime", {})
    compression_cfg["roi_detection"]["runtime"]["device"] = "cuda:0"
    compression_cfg["roi_detection"]["runtime"]["half"] = True
    compression_cfg.setdefault("compression", {}).setdefault("dcvc", {})
    compression_cfg["compression"]["dcvc"]["device"] = "cuda"
    compression_cfg["compression"]["dcvc"]["use_cuda"] = True
    ...
    research_cfg.setdefault("signals", {}).setdefault("amt_risk", {})
    research_cfg["signals"]["amt_risk"]["weights_path"] = str((repo_root / "models/amt-s.pth").resolve())
    research_cfg["signals"]["amt_risk"]["repo_dir"] = str((repo_root / "_third_party_amt").resolve())
